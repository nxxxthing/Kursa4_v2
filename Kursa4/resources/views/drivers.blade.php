<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="../../public/style.css">
  <link rel="stylesheet" href="../../public/max-width-1024px.css">
  <link rel="stylesheet" href="../../public/max-width-768px.css">
  <link rel="stylesheet" href="../../public/max-width-576px.css">
</head>
<body class="page">


  <header>
  <div class = "allHeader">
  <div class = "Header">
  <p>Треба поїздка?</p>
  <div class = "headerYelEl"><p>Замовити машину</p></div>
  <p>8-800-555-35-35</p>
  </div>
  </div>
  <div class = "allMenu">
  <div class = "Menu">
  <div class = "logoANDtext">
  <img class ="logo" src = "../../public/page1/disability_1.png">
  <a href = "web.blade.php">FakeTaxi</a>
  </div>
  <div class = "menu"><p><a href="clients.blade.php">Клієнтам</a></p></div>
  <div class = "menu"><p><a href="drivers.blade.php">Водіям</a></p></div>
  <div class = "menu"><p><a href="cities.blade.php">Міста</a></p></div>
  <div class = "menu"><p><a href="contacts.blade.php">Контакти</a></p></div>
  </div>
  </div>


  <div class = "allHeaderMobile">
    <div class = "Header">
    <p>Треба поїздка?</p>
    <div class = "headerYelEl"><p>Замовити машину</p></div>
    <p>8-800-555-35-35</p>
    </div>
    </div>
    <div class = "allMenuMobile">
    <div class = "Menu">
    <div class = "logoANDtext">
    <img class ="logo" src = "../../public/page1/disability_1.png">
    <a href = "web.blade.php">FakeTaxi</a>
    </div>
    <img class = "burger" src = "../../public/page1/image_14_1.png">
    </div>
    </div>
  </header>




  <p class = "instructionName">Переваги роботи водієм з нами</p>
  <div class = "element">
  <img class = "pic" src = "../../public/page3/1.png">
  <div class = "list">
  <p class = "title">Заробляй більше грошей</p>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Справедлива комісія</p>
  </div>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Гідний тариф</p>
  </div>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Грошові бонуси та винагороди</p>
  </div>
  </div>
  </div>

  <div class = "elementRev">
  <img class = "pic" src = "../../public/page3/2.png">
  <div class = "list">
  <p class = "title">Простіші умови</p>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Вільний графік роботи</p>
  </div>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Відсутність обов’язкових замовлень</p>
  </div>
  <div class = "listElement">
  <img src ="../../public/page1/check-mark_6.png">
  <p class = "textList">Відсутність вступних платежів</p>
  </div>
  </div>
  </div>

    <div class = "element">
    <img class = "pic" src = "../../public/page3/3.png">
    <div class = "list">
    <p class = "title">Більше можливостей</p>
    <div class = "listElement">
    <img src ="../../public/page1/check-mark_6.png">
    <p class = "textList">Відкрита інформація про замовлення</p>
    </div>
    <div class = "listElement">
    <img src ="../../public/page1/check-mark_6.png">
    <p class = "textList">Клієнти з рейтингами та відгуками</p>
    </div>
    <div class = "listElement">
    <img src ="../../public/page1/check-mark_6.png">
    <p class = "textList">Пряма комунікація з пасажиром</p>
    </div>
    </div>
    </div>

        <div class = "elementRev">
        <img class = "pic" src = "../../public/page3/4.png">
        <div class = "list">
        <p class = "title">Надійність та впевненість</p>
        <div class = "listElement">
        <img src ="../../public/page1/check-mark_6.png">
        <p class = "textList">Безпечна співпраця</p>
        </div>
        <div class = "listElement">
        <img src ="../../public/page1/check-mark_6.png">
        <p class = "textList">Швидка підтримка</p>
        </div>
        <div class = "listElement">
        <img src ="../../public/page1/check-mark_6.png">
        <p class = "textList">Ми на зв’язку 24/7</p>
        </div>
        </div>
        </div>



  <div class = "Block--greygrey">
  <p class = "instructionName">Як стати нашим водієм</p>
  <div class = "TotalServiceBlock">
  <div class = "serviceBlock--drivers">
  <img class = "el" src ="../../public/page3/5.png">
  <p class = "serviceText--drivers">Мати автомобіль</p>
  </div>
  <div class = "serviceBlock--drivers">
  <img class = "el" src ="../../public/page3/6.png">
  <p class = "serviceText--drivers">Мати смартфон або планшет</p>
  </div>
  <div class = "serviceBlock--drivers">
  <img class = "el" src ="../../public/page3/7.png">
  <p class = "serviceText--drivers">Зареєструватися на нашому сайті</p>
  </div>
  </div>
  </div>







  <div class = "Block--black">
  <p class = "Contacts">Контакти</p>
  <div class = "allCont">
  <p class = "adress">Адреса <br> вул. Політехнічна, 12 <br><br><br> Телефон <br>8-800-555-35-35</p>
  <div class = "contact">
  <p class = "contacts">Email <br> fakeTaxi@gmail.com<br><br>Ми в соціальних мережах</p>
  <div class = "icon">
  <img class = "icon" src ="../../public/page1/check-mark_7.svg">
  <img class = "icon" src ="../../public/page1/check-mark_8.svg">
  <img class = "icon" src ="../../public/page1/check-mark_9.svg">
  <img class = "icon" src ="../../public/page1/check-mark_10.svg">
  </div>
  </div>
  </div>
  </div>

</body>
 </html>
